...
module.exports = class proxyProductionPlugin {
  constructor(options) {
    ...
    this.app = options.host.startsWith('https') ? https : http;
    this.port = options.port;
    this.defaultPort = options.host.startsWith('https') ? 443 : 80;
      ...
      (complication, callback) => {
        ...
        assetsKeys.forEach(key => {
          if (!this.map[key]) {
            ...
            if (this.publicPath) {
              let baseUrl = this.publicPath + '/static';
              if (!this.publicPath.startsWith('http')) {
                baseUrl = '/' + baseUrl;
              }
      ...
      .listen(this.port, err => {
        if (!err) {
          this.messages.forEach(url => {
            console.log('Hit Ctrl To Open Url: ', url + '\n');
          });
        }
      });
  }
  ...
  [proxyResponse](req, res, path, query, data) {
    ...
    const proxyReq = this.app.request({
        method: req.method,
        protocol: options.protocol,
        host: options.host,
        hostname: options.host,
        port: options.port ? Number(options.port) : this.defaultPort,
        path: pathUrl,
        timeout: 5000,
